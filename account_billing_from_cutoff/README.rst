...
-  An invoice without a recipient bank does not state where it is to be
   ...
   partner and currency that do have one, instead of getting a billing
   of its own.
-  When those invoices point to several recipient banks, the first bank
   is used, which is the one the invoice would have been given by
   default.
